...
class ActivityDashboard extends ConsumerStatefulWidget {
  final VoidCallback? onConversionRestored;

  const ActivityDashboard({super.key, this.onConversionRestored});

  @override
  ConsumerState<ActivityDashboard> createState() => _ActivityDashboardState();
}

class _ActivityDashboardState extends ConsumerState<ActivityDashboard> {
  static const int _initialItemsCount =
      5; // Reduced initial count for embedded view
    ...
    return Column(
      ...
      children: [
        ...
        heatmapDataAsync.when(
          data: (heatMapDatasets) => _heatMapCard(context, heatMapDatasets),
          loading: () => _buildLoadingCard(context),
          error: (error, stack) => _buildErrorCard(context, error),
        ),
        const SizedBox(height: AppTheme.spacingLg),
        conversionsAsync.when(
          data: (conversions) => _buildArchiveSection(context, conversions),
  ...
  Future<void> _loadInConverter(
    ...
    // Call optional callback if provided, e.g. to scroll up
    widget.onConversionRestored?.call();
  }
}
