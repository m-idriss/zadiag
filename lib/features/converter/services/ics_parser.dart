...
class IcsParser {
  ...
  static CalendarEvent? _parseVEvent(String veventBlock, int index) {
    try {
      ...
      final uid = properties['UID'] ?? '${DateTime.now().millisecondsSinceEpoch}-$index';
      ...
      return CalendarEvent(
        ...
        endDateTime: dtEnd,
        location: location != null && location.isNotEmpty ? _unescapeIcsText(location) : null,
        isAllDay: isAllDay,
  ...
  static DateTime _parseDateTimeProperty(Map<String, String> properties, String baseName) {
  ...
  static bool _hasDateOnlyParameter(Map<String, String> properties, String baseName) {
    for (final key in properties.keys) {
      if (key.startsWith('$baseName;VALUE=DATE')) {
        return true;
      }
