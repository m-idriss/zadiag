...
export const buildDiagnosticsEmailBody = (input: DiagnosticsInput): string => {
  ...
  const appLines = [
    ...
    `PushLastReceivedAt: ${input.pushHealth?.lastPushReceivedAt ?? 'missing'}`,
    `PushDeliveryStatus: ${input.pushHealth?.deliveryStatus ?? 'missing'}`,
    `PushRecoveryRequired: ${String(input.pushHealth?.recoveryRequired ?? false)}`,
