...
describe('adherence report CSV', () => {
  it('creates one stable database-like row per event', async () => {
    ...
    expect(lines[0]).toBe('participant,period,exported_at,event_id,session_id,routine_id,routine_name,requested_at,expires_at,captured_at,status,analysis_source,automated_status,confidence,image_quality,reason,review_status,reviewed_at,review_reason,responsible_actions');
