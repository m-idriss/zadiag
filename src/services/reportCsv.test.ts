  ...
  it('creates one stable database-like row per event', async () => {
    const blob = createAdherenceReportCsv({
      ...
      events: [{
        ...
        responsibleActions: [{ type: 'approved', at: '2026-07-15T08:10:00.000Z', actorUid: 'owner-1', actorName: 'Idriss' }],
        anomaly: 'rejected',
      }],
    });
    ...
    expect(lines[0]).toBe('participant,period,exported_at,event_id,session_id,routine_id,routine_name,requested_at,expires_at,captured_at,status,analysis_source,automated_status,confidence,image_quality,reason,review_status,reviewed_at,review_reason,responsible_actions,routine_anomaly');
    ...
    expect(lines[1]).toContain('Idriss');
