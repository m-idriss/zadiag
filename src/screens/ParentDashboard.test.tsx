...
describe('ParentDashboard', () => {
  ...
  it('keeps the parent overview blocks and replaces needs attention with filterable history', () => {
    ...
    const individualSummary = container.querySelector('.adherence-summary-card');
    const individualOverview = container.querySelector('.participant-switcher-static');
    expect(individualSummary).not.toBeNull();
    expect(individualOverview).not.toBeNull();
    expect(Boolean(individualSummary!.compareDocumentPosition(individualOverview!) & Node.DOCUMENT_POSITION_FOLLOWING)).toBe(true);
  ...
  it('reuses the individual dashboard with participant filters and context labels', () => {
    ...
    const adherenceSummary = container.querySelector('.adherence-summary-card');
    const participantOverview = container.querySelector('.participant-switcher');
    expect(adherenceSummary).not.toBeNull();
    expect(participantOverview).not.toBeNull();
    expect(Boolean(adherenceSummary!.compareDocumentPosition(participantOverview!) & Node.DOCUMENT_POSITION_FOLLOWING)).toBe(true);
    const secondaryFilters = container.querySelector<HTMLDetailsElement>('.history-secondary-filters');
    const participantFilter = container.querySelector('.filter-group:has(.participant-filter-chip)');
    expect(secondaryFilters?.open).toBe(false);
    expect(secondaryFilters?.querySelector('summary')?.textContent).toBe('History filters');
    expect(secondaryFilters?.contains(participantFilter ?? null)).toBe(false);
    expect(secondaryFilters?.querySelectorAll('.filter-group')).toHaveLength(2);
